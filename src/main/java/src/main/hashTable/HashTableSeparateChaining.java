...
class Entry<K, V> {
    ...
    // the reason is to avoid casting
...
public class HashTableSeparateChaining<K, V> implements Iterable<K> {
    ...
    // its a maximum capacity that
    // we are willing to tolerate
    private static final double DEFAULT_LOAD_FACTOR = 0.75;
    // once the load factor goes above this value
    // we reset the table
    private double maxLoadFactor;
    // threshold = capacity * maxLoadFactor
    // it tells: when is the time to resize
